...
const PLANS = [
  {
    ...
    bestFor: 'Students, early professionals, and occasional high-stakes messages.',
    weeklyPrice: null,
    monthlyPrice: 7.99,
    yearlyPrice: 79,
    ...
    tagline: 'Affordable support for everyday professional replies when you don\'t want to get it wrong.',
    weeklyNote: null,
    features: [
  ...
  },
  {
    ...
    bestFor: 'Sales conversations, customer support, work situations, and dating.',
    weeklyPrice: 5.99,
    monthlyPrice: 14.99,
    yearlyPrice: 149,
  ...
  },
  {
    ...
    bestFor: 'Operators, founders, agency owners, executives and revenue leaders.',
    weeklyPrice: null,
    monthlyPrice: 24.99,
    yearlyPrice: 249,
    ...
    tagline: 'Advanced protection for high-stakes communication that drives results.',
    weeklyNote: null,
    features: [
...
function PlanCard({ plan, billing, onSignup }) {
  ...
  const price = showWeekly ? plan.weeklyPrice : isYearly ? plan.yearlyPrice : plan.monthlyPrice
  ...
  return (
    ...
    onMouseLeave={e => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = isPopular ? `0 0 0 1px ${plan.color}20, 0 12px 40px rgba(0,0,0,0.15)` : 'none' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
        <div style={{ width: 34, height: 34, borderRadius: 9, background: isPopular ? `${plan.color}18` : 'var(--surface2)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
          <Icon size={16} color={isPopular ? plan.color : 'var(--ink-3)'} />
        </div>
        <span style={{ fontSize: 20, fontWeight: 800, letterSpacing: '-0.03em', color: 'var(--ink)' }}>{plan.name}</span>
      </div>
      <p style={{ fontSize: 13, color: 'var(--ink-3)', lineHeight: 1.45, marginBottom: 20 }}><em>{plan.desc}</em></p>

      {/* Price */}
      <div style={{ marginBottom: 6 }}>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 4 }}>
          <span style={{ fontSize: 13, color: 'var(--ink-3)', alignSelf: 'flex-start', marginTop: 8 }}>$</span>
          <span style={{ fontSize: 44, fontWeight: 800, letterSpacing: '-0.05em', color: 'var(--ink)' }}>{price}</span>
          <span style={{ fontSize: 13, color: 'var(--ink-3)' }}>{priceUnit}</span>
        </div>
        ...
        {isYearly && (
          <p style={{ fontSize: 12, color: 'var(--green)', fontWeight: 600, marginTop: 3 }}>Save 2 months · Pay for 10, get 12</p>
        )}
        {isWeekly && !plan.weeklyPrice && (
          <p style={{ fontSize: 12, color: 'var(--ink-4)', fontStyle: 'italic', marginTop: 3 }}>Weekly billing available on Daily plan only.</p>
        )}
      </div>

      {/* Replies highlight */}
      <p style={{ fontSize: 13, fontWeight: 700, color: isPopular ? plan.color : 'var(--ink)', marginBottom: 6 }}>{plan.repliesNote.split('(')[0].trim()}</p>
      <p style={{ fontSize: 12, color: 'var(--ink-4)', marginBottom: 14 }}>({plan.repliesNote.split('(')[1]?.replace(')', '') || ''})</p>

      {/* Tagline */}
      <p style={{ fontSize: 13.5, color: 'var(--ink-3)', lineHeight: 1.6, marginBottom: 20, paddingBottom: 18, borderBottom: '1px solid var(--border)' }}>{plan.tagline}</p>

      {/* Features */}
      <p style={{ fontSize: 10.5, fontWeight: 700, color: 'var(--ink-4)', textTransform: 'uppercase', letterSpacing: '0.09em', marginBottom: 10 }}>Includes</p>
      <ul style={{ listStyle: 'none', display: 'flex', flexDirection: 'column', gap: 8, flex: 1 }}>
        {plan.features.map(f => (
          <li key={f.text} style={{ display: 'flex', alignItems: 'flex-start', gap: 9 }}>
            <div style={{ width: 17, height: 17, borderRadius: 5, flexShrink: 0, marginTop: 1, background: `${isPopular ? plan.color : 'var(--green)'}15`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <Check size={10} color={isPopular ? plan.color : 'var(--green)'} strokeWidth={2.5} />
            </div>
            <span style={{ fontSize: 13.5, color: 'var(--ink)', lineHeight: 1.45 }}>{f.text}</span>
          </li>
        ))}
      </ul>

      {/* Best for */}
      <p style={{ fontSize: 12, color: 'var(--ink-3)', marginTop: 16, marginBottom: 18, lineHeight: 1.5 }}>
        <strong style={{ color: 'var(--ink-2)' }}>Best for:</strong> {plan.bestFor}
      </p>
...
function FAQItem({ item }) {
  ...
  return (
    <div style={{ border: `1px solid ${open ? 'rgba(45,212,191,0.25)' : 'var(--border)'}`, borderRadius: 14, background: open ? 'rgba(45,212,191,0.03)' : 'var(--surface)', overflow: 'hidden', transition: 'border-color .2s' }}>
      <button onClick={() => setOpen(o => !o)} style={{ width: '100%', padding: '16px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, background: 'none', border: 'none', fontFamily: 'inherit', cursor: 'pointer', textAlign: 'left' }}>
        <span style={{ fontSize: 15, fontWeight: 600, color: 'var(--ink)', letterSpacing: '-0.015em', lineHeight: 1.4 }}>{item.q}</span>
        <div style={{ width: 28, height: 28, borderRadius: 8, background: open ? '#2dd4bf' : 'var(--surface2)', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'background .2s' }}>
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg)' }}>
      ...
      <header style={{ position: 'sticky', top: 0, zIndex: 50, background: 'var(--nav-bg)', backdropFilter: 'blur(20px)', borderBottom: '1px solid var(--border)' }}>
        <div className="container" style={{ display: 'flex', alignItems: 'center', height: 60, gap: 12 }}>
          <button onClick={onBack} style={{ display: 'flex', alignItems: 'center', gap: 5, color: 'var(--ink-3)', fontSize: 13, transition: 'color .15s', background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'inherit' }}
      ...
      <div className="container" style={{ paddingTop: 'clamp(40px,6vw,72px)', paddingBottom: 80 }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 'clamp(36px,5vw,56px)' }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '4px 14px', borderRadius: 999, background: 'rgba(45,212,191,0.08)', border: '1px solid rgba(45,212,191,0.2)', marginBottom: 18 }}>
            <Star size={12} color="#2dd4bf" />
            <span style={{ fontSize: 12, fontWeight: 700, color: '#2dd4bf' }}>Start with a 7-day free trial. No credit card required.</span>
          </div>
          <h1 style={{ fontSize: 'clamp(28px,5vw,52px)', fontWeight: 800, letterSpacing: '-0.04em', lineHeight: 1.08, marginBottom: 14 }}>
            Choose how often you want<br />
            <span style={{ background: 'linear-gradient(135deg,#2dd4bf,var(--green))', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>Avertune on your side.</span>
          </h1>

          {/* Billing toggle */}
          <div style={{ display: 'inline-flex', alignItems: 'center', padding: '4px', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, gap: 2, marginTop: 8 }}>
            {[
              ...
              <button key={b.id} onClick={() => setBilling(b.id)} style={{
                padding: '7px 16px', borderRadius: 9, fontFamily: 'inherit', fontWeight: 600, fontSize: 13.5, border: 'none', cursor: 'pointer',
                background: billing === b.id ? 'var(--surface2)' : 'transparent',
                color: billing === b.id ? 'var(--ink)' : 'var(--ink-3)',
                transition: 'all .15s',
                boxShadow: billing === b.id ? '0 1px 6px rgba(0,0,0,0.15)' : 'none',
              }}>{b.label}</button>
            ))}
          </div>
          {billing === 'weekly' && (
            <p style={{ fontSize: 12.5, color: 'var(--ink-3)', marginTop: 10 }}>Weekly Pass — available for Daily plan only. Billed every 7 days.</p>
          )}
          {billing === 'annual' && (
            <p style={{ fontSize: 12.5, color: 'var(--ink-3)', marginTop: 10 }}>Pay for 10 months, get 12. Lock in your rate.</p>
          )}
        </div>
        ...
        <div style={{ marginBottom: 'clamp(56px,8vw,88px)' }}>
          <div style={{ textAlign: 'center', marginBottom: 32 }}>
            <h2 style={{ fontSize: 'clamp(22px,3.5vw,32px)', fontWeight: 800, letterSpacing: '-0.03em', marginBottom: 8 }}>Need more replies?</h2>
            <p style={{ fontSize: 14.5, color: 'var(--ink-3)' }}>Buy extra reply packs when you run out early.</p>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(min(100%,220px),1fr))', gap: 14, maxWidth: 720, margin: '0 auto' }}>
            {TOPUPS.map(t => (
              <div key={t.replies} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 16, padding: '24px 20px', textAlign: 'center', transition: 'border-color .2s' }}
        ...
        <div style={{ maxWidth: 700, margin: '0 auto' }}>
          <h2 style={{ fontSize: 'clamp(22px,4vw,36px)', fontWeight: 800, letterSpacing: '-0.04em', textAlign: 'center', marginBottom: 32 }}>Common Questions</h2>
